...
public class LocationTest {
    ...
    public void testAllocateSupplyToVictim() {
        location.addOccupant(victim);
        location.addSupply(supply); 
        location.allocateSupplyToVictim(supply, victim);
        assertTrue("allocateSupplyToVictim should add the supply to the victim's personal belongings", victim.getPersonalBelongings().contains(supply));
    }

    @Test
    public void testDeallocateSupplyFromVictim() {
        location.addOccupant(victim); 
        location.addSupply(supply); 
        location.allocateSupplyToVictim(supply, victim); 
        location.deallocateSupplyFromVictim(supply, victim);
        assertFalse("deallocateSupplyFromVictim should remove the supply from the victim's personal belongings", victim.getPersonalBelongings().contains(supply));
    }
}
